...
namespace NameRecorder
{
    ...
    class DataManager
    {
        ...
        void createTable()
        {
            private string sqluniqname = "create table unique_name (reference_name text, unique_name_id int)";
            SQLiteCommand command = new SQLiteCommand(sqluniqname, m_dbConnection);
            command.ExecuteNonQuery();
            private string sqlrec = "create table recordings (rec_id int, unique_name_id from unique_name, sample_rate int, rec_voice_id from recorder_voices, timestamp datetime, filepath text)";
            command = new SQLiteCommand(sqlrec, m_dbConnection);
            command.ExecuteNonQuery();
           private  string sqlallname = "create table all_names (reference_name text, id int)";
            command = new SQLiteCommand(sqlallname, m_dbConnection);
            command.ExecuteNonQuery();
            private string sqlrecvoice = "create table recorder_voices (rec_voice_id int ,voice_name text)";
            command = new SQLiteCommand(sqlrecvoice, m_dbConnection);
            command.ExecuteNonQuery();
        }

        // Inserts some test values in a table.
        //  duplicate code here solve later.
        void fillTable()
        {
